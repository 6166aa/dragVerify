(function () {
  ...
  function init(el, option) {
    ...
    fields.sliderBackground = el.querySelector(
      "[class^=drag-verify__slider-background]"
    );
    fields.imgBackground = el.querySelector(
      "[class^=drag-verify__img-background]"
    );
    ...
    el.querySelector("[class^=drag-verify__close]").addEventListener(
      "click",
      closeVerifyModel
    );
    fields.sliderBlock.addEventListener("mousedown", mouseDown);
    el.querySelector("[class^=drag-verify__reset]").addEventListener(
      "click",
      resetVerify
    );
  ...
  function mouseDown(event) {
    ...
    fields.isDown = true;
    console.log("mouseDown.x", fields.x);
    //监听移动
  }
  function mouseUp() {
    if (fields.isDown) {
      ...
      fields.isDown = false;
      //执行回调
      verify(fields.offsetX, fields.verifyId, (data) => {
        if (data.success) {
          verifySuccess();
        } else {
          verifyFail();
  ...
  function showVerifyModel() {
    getVerifyInfo((data) => {
      setVerifyData(data);
      fields.verifyModel.className = "drag-verify__model--show";
      //获取滑块偏移最大范围
      if (!fields.maxOffsetX) {
        fields.maxOffsetX =
          fields.el.querySelector(".drag-verify__slider").offsetWidth -
          fields.sliderBlock.offsetWidth;
      }
      //获取图片拼图最大范围
      if (!fields.maxImgOffsetX) {
        fields.maxImgOffsetX =
          fields.el.querySelector(".drag-verify__img").offsetWidth -
          fields.imgBlock.offsetWidth;
      }
      fields.failCount = 0;
    });
  }
  ...
  function verifySuccess() {
    ...
    fields.sliderBackground.className =
      "drag-verify__slider-background--success";
    setTimeout(function () {
      closeVerifyModel();
    }, 1000);
  }
